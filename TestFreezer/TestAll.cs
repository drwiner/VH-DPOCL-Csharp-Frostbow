...
namespace TestFreezer
{
    class TestAll
    {
        ...
        {
            var POP = new PlanSpacePlanner(initPi, SelectMethod, SearchMethod, false)
            {
        ...
        public static void RunTravelTestInternal()
        {
            ...
            for (int i = 7; i < 8; i++)
            {
                ...
                //RunPlanner(initPlan.Clone() as IPlan, new ADstar(), new E3Star(new ZeroHeuristic(), .5f), k, cutoff, directory, i);
                //RunPlanner(initPlan.Clone() as IPlan, new ADstar(), new E3Star(new NumOpenConditionsHeuristic(), .5f), k, cutoff, directory, i);
